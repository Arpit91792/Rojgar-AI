// Dashboard — all stats come from localStorage via DataContext.
// FUTURE: replace getStats() with adminApi.get('/api/admin/stats')
...
import {
      ...
      FileText, FileCheck, BarChart3, FileEdit, Eye, Archive,
} from 'lucide-react'
...
const AdminDashboard = () => {
      const { admin } = useAuth()
      const { posts, getStats } = useData()
      const stats = getStats()

      // 5 most recent posts across all categories
                                          ...
                                          <tbody className="divide-y divide-gray-100">
                                                {recentPosts.map((post) => (
                                                      <tr key={post.id} className="hover:bg-gray-50">
                                                            ...
                                                            <td className="px-4 py-3 text-gray-500 hidden sm:table-cell text-xs">{post.category?.replace('_', ' ')}</td>
