...
const ResultCard = ({ post }) => {
      ...
      return (
            ...
            >
                  ...
                  {post.organization && (
                        <p className="text-gray-500 text-xs mb-3 truncate">{post.organization}</p>
                  )}
                  ...
                  <div className="space-y-1 text-xs text-gray-500 mt-auto">
                        ...
                        {lastDate && (
                              <div className="flex items-center gap-1.5">
                                    ...
                                    <span>
                                          {post.lastDate ? 'Last Date: ' : post.examDate ? 'Exam: ' : 'Date: '}
                                          <span className="font-medium text-red-500">{fmt(lastDate)}</span>
                                    </span>
                              </div>
                        )}
                        {(post.salary || post.stipend) && (
                              <p className="font-medium text-green-600">{post.salary || post.stipend}</p>
                        )}
                  </div>
...
const SearchPage = () => {
      ...
      useEffect(() => {
            try {
                  const published = postService.getPosts().filter((p) => p.status === 'PUBLISHED')
            ...
            const handleUpdate = () => {
                  try {
                        setAllPosts(postService.getPosts().filter((p) => p.status === 'PUBLISHED'))
                  } catch (_) { }
            }
                              ...
                              {results.length === 0 ? (
                                    <div className="bg-white rounded-xl p-12 text-center border">
                                          ...
                                          <p className="text-gray-500 text-sm">
                                                Try different keywords or browse a category from the sidebar.
                                          </p>
                                    </div>
                              ) : (
                                    <>
