...
const Internships = () => {
      const { posts, loading, error } = usePosts('INTERNSHIP', 'PUBLISHED')
      ...
      return (
            <div className="space-y-6">
                  <div className="flex items-center gap-3">
                        ...
                        <div>
                              <h1 className="text-2xl font-extrabold text-slate-900">Internships</h1>
                              <p className="text-sm text-slate-500">Launch your career with hands-on experience</p>
                        </div>
                        {!loading && (
                  ...
                  <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-sm">
                        <div className="flex gap-3">
                              <div className="relative flex-1">
                                    ...
                                    <input
                                          type="text"
                                          placeholder="Search by title, company, skills, eligibility…"
                                          value={search}
                  ...
                  {!loading && !error && filtered.length > 0 && (
                        <div className="flex flex-col gap-4">
                              {filtered.map((item) => <JobCard key={item.id} job={item} />)}
                        </div>
                  )}
                  ...
                  {!loading && !error && filtered.length === 0 && (
                        <EmptyState
                              ...
                              desc={posts.length === 0 ? 'No internships published yet.' : 'Try adjusting your search.'}
