...
import { FileCheck, Search, X, CalendarDays, Clock, Share2, Heart, Download, AlertCircle } from 'lucide-react'
            ...
            >
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-3 text-xs text-slate-500">
                              {item.examDate && (
                                    <span className="flex items-center gap-1.5">
                                          ...
                                          Exam Date: <span className="font-medium text-slate-700 ml-1">{fmt(item.examDate)}</span>
                                    </span>
                              )}
                              {item.releaseDate && (
                                    <span className="flex items-center gap-1.5">
                                          <Clock size={13} className="text-slate-400" />
                                          Released: <span className="font-medium text-slate-700 ml-1">{fmt(item.releaseDate)}</span>
                                    </span>
                              )}
                              {item.qualification && (
                  ...
                  <div className="px-5 py-3 flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-1" onClick={(e) => e.preventDefault()}>
                              ...
                              {item.downloadLink ? (
                                    <a href={item.downloadLink} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
