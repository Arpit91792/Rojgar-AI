...
// Category-to-route mapping used to highlight the active category on post detail
const CAT_ROUTE = {
...
// ── Horizontal category nav — shown in topbar on /posts/:slug ─────────────────
...
const Header = ({ postCategory }) => {
      ...
      // Are we on a post detail page?
      const isPostDetail = /^\/posts\/[^/]+$/.test(location.pathname)

      // Which category route should be highlighted (passed from PostDetail)
      const activeCatRoute = postCategory ? (CAT_ROUTE[postCategory] || null) : null
                  ...
                  <header className="fixed top-0 left-0 right-0 z-30 glass border-b border-slate-200/80 shadow-sm">
                        <div className="flex items-center justify-between h-16 w-full px-4 sm:px-6">
                              ...
                              <div className="flex items-center gap-1 flex-shrink-0">
                                    {/* Mobile search icon — hidden on post detail since we have category nav there */}
                                    {!isPostDetail && (
                                          <button
                                                aria-label="Search"
                        ...
                        {/* Mobile search slide-down — only on non-post-detail pages */}
                        {!isPostDetail && (
                              <div className={[
                        ...
                        {/* Mobile horizontal category strip — only on post detail pages */}
                        {isPostDetail && (
                              <nav
                                    className="md:hidden flex items-center gap-1 px-3 pb-2.5 overflow-x-auto scrollbar-hide border-t border-slate-100"
                                    aria-label="Browse categories"
                              >
                                    {NAV_ITEMS.map(({ icon: Icon, label, to, accent }) => {
                                          const isActive = activeCatRoute === to
                                          return (
                                                ...
                                                >
                                                      <Icon size={11} className={isActive ? 'text-white' : accent} />
                                                      {label}
                                                </Link>
                                          )
                                    })}
